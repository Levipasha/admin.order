...
export default function SuperAdminPanel({
  ...
  const fetchLedger = async () => {
    ...
    if (!token) {
      setLedgerError('Live backend token is missing. Please log out and log back in.');
    ...
    try {
      ...
      const res = await fetch(`${apiBase}/super-admin/payouts/ledger`, {
        headers: {
      ...
      if (data.success && data.ledger) {
        ...
        setLedgerError(data.error || 'Failed to fetch payout ledger.');
      }
    } catch (err) {
  ...
  const handleSendPayoutSubmit = async (e) => {
    ...
    try {
      const apiBase = import.meta.env.VITE_API_BASE || 'http://localhost:5000/api';
      const res = await fetch(`${apiBase}/super-admin/payouts`, {
        method: 'POST',
        headers: {
      ...
      if (data.success) {
        ...
        alert('Failed to log payout: ' + (data.error || 'Unknown error'));
      }
    } catch (err) {
  ...
  const handleToggleApproveRestaurant = async (restId, currentState) => {
    const token = localStorage.getItem('Orderin_super_token');
    try {
      const apiBase = import.meta.env.VITE_API_BASE || 'http://localhost:5000/api';
      const response = await fetch(`${apiBase}/super-admin/restaurants/${restId}/status`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': token ? `Bearer ${token}` : ''
      ...
      if (data.success && data.restaurant) {
        setRestaurants(restaurants.map(r => (r.id === restId || r._id === restId) ? { 
          ...r, 
          isApproved: data.restaurant.isApproved 
        } : r));
        alert(`Restaurant approval status updated successfully to ${data.restaurant.isApproved ? 'Approved' : 'Pending'}`);
      } else {
        alert('Failed to update restaurant status: ' + (data.error || 'Unknown error'));
      }
    } catch (err) {
  ...
  const handleToggleActiveRestaurant = async (restId, currentState) => {
    const token = localStorage.getItem('Orderin_super_token');
    try {
      const apiBase = import.meta.env.VITE_API_BASE || 'http://localhost:5000/api';
      const response = await fetch(`${apiBase}/super-admin/restaurants/${restId}/status`, {
      ...
      if (data.success && data.restaurant) {
        ...
        alert(`Restaurant active status updated successfully to ${data.restaurant.isActive ? 'Active' : 'Disabled'}`);
      } else {
        alert('Failed to update restaurant status: ' + (data.error || 'Unknown error'));
      }
    } catch (err) {
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <ResponsiveContainer width="100%" height="100%" minHeight={250}>
                      <BarChart
                        data={restaurants.map(r => ({
                          name: (r.name || 'Rest').substring(0, 8) + '..',
                          orders: orders.filter(o => o.restaurantId === r.id || o.restaurantId === r._id).length
                        }))}
                      >
                ...
                <div className="bg-slate-900 border border-white/5 rounded-3xl p-6 flex flex-col justify-center">
                  ...
                  <div className="h-56 flex items-center justify-center w-full" style={{ minHeight: '224px' }}>
                        ...
                        <td className="p-4 space-y-1">
                          <div className="flex items-center gap-1.5">
                            <span className={`px-2 py-0.5 rounded text-[8px] font-black uppercase ${
                              rest.subscriptionPlan === 'free'
                                ? 'bg-amber-500/10 text-amber-400 border border-amber-500/20'
                                : 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20'
                            }`}>
                              {rest.subscriptionPlan === 'free' ? 'Free Trial' : 'Premium Pro'}
                            </span>
                            <span className={`px-1.5 py-0.5 rounded text-[8px] font-black uppercase ${
                              (!rest.subscriptionActive || (rest.subscriptionExpiry && new Date(rest.subscriptionExpiry) < new Date()))
                                ? 'bg-red-500/10 text-red-500 border border-red-500/20'
                                : 'bg-green-500/10 text-green-400 border border-green-500/20'
                            }`}>
                              {(!rest.subscriptionActive || (rest.subscriptionExpiry && new Date(rest.subscriptionExpiry) < new Date())) ? 'Hold' : 'Active'}
                            </span>
                          </div>
                          <span className="text-[10px] text-slate-400 block font-mono">
                            {rest.subscriptionExpiry 
                              ? new Date(rest.subscriptionExpiry).toLocaleDateString('en-IN', { day: 'numeric', month: 'short' })
                              : 'No Expiry'
                            }
                          </span>
                        </td>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-1">
                  ...
                  >
                    ...
                    <option value="basic">Premium Pro (₹1/mo — test)</option>
                  </select>
                </div>
              </div>
